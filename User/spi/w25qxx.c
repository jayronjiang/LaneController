...
void   SPI_M25PE_WRLR(uint32 Dst_Addr,uchar byte)    
{
	...
	W25QXX_CS=0;                            //使能器件   
	SPI2_ReadWriteByte(M25PE_WRLR);                   //  fast write command    
	SPI2_ReadWriteByte(((Dst_Addr & 0xFFFFFF) >> 16));     //  send 3 address bytes    
	SPI2_ReadWriteByte(((Dst_Addr & 0xFFFF) >> 8));   
	SPI2_ReadWriteByte(Dst_Addr & 0xFF);     
    	Temp = SPI2_ReadWriteByte();   
    	W25QXX_CS = 1;                                        //  disable device      
    	return Temp;                                        //  return one byte read    
}
#endif
...
uint8_t  SPI_Read_ID1(void)
{
	uint8_t Temp = 0;	  
	W25QXX_CS=0;                            //使能器件   
	SPI2_ReadWriteByte(W25X_DeviceID); //发送读取状态寄存器命令 
	SPI2_ReadWriteByte(0x00);
	SPI2_ReadWriteByte(0x00);
	SPI2_ReadWriteByte(0x00);
	Temp =SPI2_ReadWriteByte(0Xff);          //读取一个字节  
	W25QXX_CS=1;                            //取消片选     
	return Temp;
}   
...
{   
    	...
    	W25QXX_Wait_Busy();   
  	W25QXX_CS=0;                            	//使能器件   
    	SPI2_ReadWriteByte(W25X_BlockErase);      	//发送扇区擦除指令 
   	SPI2_ReadWriteByte((uint8_t)((Dst_Addr & 0xFFFFFF)>>16));  	//发送24bit地址    
