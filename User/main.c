...
const uint32_t FLASH_SIZE=128*1024*1024;		//FLASH 大小为16M字节
...
static void Task_Schedule(void)
{
	...
	if (system_flag&SYS_CHANGED)
	{
		...
		if(Status_Get(ALARM) == 0)		// key0 按下
		{
			debug_puts("Start Read W25Q128...");
			W25QXX_Read(datatemp,0,32);//从倒数第100个地址处开始,读出SIZE个字节
